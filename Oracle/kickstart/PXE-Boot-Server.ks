...
cat<<-SYSTEMD >> /etc/systemd/system/multi-user.target.wants/var-lib-tftpboot-os-oel-7.4-x86_64.mount
[Unit]
Description=OEL 7.4 x86_64 Install Directory
Before=local-fs.target
ConditionPathExists=/dev/sr1

[Mount]
What=/dev/sr1
Where=/var/lib/tftpboot/os/oel/7.4/x86_64
Type=iso9660
Options=ro
SYSTEMD
cat<<-SYSTEMD >> /etc/systemd/system/multi-user.target.wants/var-lib-tftpboot-os-oel-6.9-x86_64.mount
[Unit]
Description=OEL 6.9 x86_64 Install Directory
Before=local-fs.target
ConditionPathExists=/dev/sr2

[Mount]
What=/dev/sr2
Where=/var/lib/tftpboot/os/oel/6.9/x86_64
Type=iso9660
Options=ro
SYSTEMD
